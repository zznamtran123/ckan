...
def package_show(context, data_dict):
    """ Package show permission checks the user group if the state
    is deleted """
    ...
    if package.state == 'deleted':
        if not user or not userobj:
            return {'success': False, 'msg':
                    _('User not authorized to read package %s') %
                    (package.id)}
    ...
    if groups:
        ...
        # We want to abort with a 404 here so we don't leak info about private
        # datasets
        abort(404)
...
def resource_show(context, data_dict):
    """ Resource show permission checks the user group if the package
    state is deleted """
    model = context['model']
    user = context.get('user')
    ...
    if group.state == 'deleted':
        if not user or \
                not _groups_intersect(userobj.get_groups('organization'),
                                      group.get_groups('organization')):
            return {'success': False, 'msg':
                    _('User {u} not authorized to show group {g}').
                    format(u=str(user), g=group.id)}
...
def get_site_user(context, data_dict):
    if not context.get('ignore_auth'):
        return {'success': False, 'msg':
                _('Only internal services allowed to use this action')}
    else:
        return {'success': True}
