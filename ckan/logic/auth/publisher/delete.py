...
def package_delete(context, data_dict):
    ...
    if not userobj or \
        ...
        return {'success': False,
                'msg':
                _('User %s not authorized to delete packages in these group')
                % str(user)}
    return {'success': True}
...
def related_delete(context, data_dict):
    ...
    if not user:
        return {'success': False, 'msg':
                _('Only the owner can delete a related item')}
...
def group_delete(context, data_dict):
    """
    Group delete permission.  Checks that the user specified is within the
    group to be deleted and also have 'admin' capacity.
    """
    model = context['model']
    user = context['user']
    ...
    if not user:
        return {'success': False, 'msg':
                _('Only members of this group are authorized to delete '
                  'this group')}
    ...
    authorized = _groups_intersect(userobj.get_groups('organization',
                                                      'admin'),
                                   [group])
    if not authorized:
        return {'success': False, 'msg':
...
def revision_undelete(context, data_dict):
    return {'success': False, 'msg':
            _('Not implemented yet in the auth refactor')}
...
def task_status_delete(context, data_dict):
    ...
    if not authorized:
        return {'success': False, 'msg':
                _('User %s not authorized to delete task_status') %
                str(user)}
    else:
        return {'success': True}
