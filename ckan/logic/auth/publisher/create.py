...
def related_create(context, data_dict=None):
    ...
    return {'success': False, 'msg':
            _('You must be logged in to add a related item')}
...
def package_relationship_create(context, data_dict):
    ...
    authorized = _groups_intersect(usergrps, pkg1grps) and \
        _groups_intersect(usergrps, pkg2grps)
    if not authorized:
        return {'success': False, 'msg':
                _('User %s not authorized to edit these packages') %
                str(user)}
    else:
        return {'success': True}
...
def group_create(context, data_dict=None):
    """
    Group create permission.  If a group is provided, within which we want to
    create a group then we check that the user is within that group.  If not
    then we just say Yes for now although there may be some approval issues
    elsewhere.
    """
    model = context['model']
    user = context['user']
    ...
    if not model.User.get(user):
        return {'success': False, 'msg':
                _('User is not authorized to create groups')}

    if Authorizer.is_sysadmin(user):
        return {'success': True}

    try:
        # If the user is doing this within another group then we need to
        # make sure that the user has permissions for this group.
    ...
    if not userobj:
        return {'success': False, 'msg':
                _('User %s not authorized to create groups') % str(user)}
...
def authorization_group_create(context, data_dict=None):
    return {'success': False, 'msg':
            _('Authorization groups not implemented in this profile') %
            str(user)}
...
def package_create_rest(context, data_dict):
    ...
    if user in (model.PSEUDO_USER__VISITOR, ''):
        return {'success': False, 'msg':
                _('Valid API key needed to create a package')}
...
def group_create_rest(context, data_dict):
    ...
    if user in (model.PSEUDO_USER__VISITOR, ''):
        return {'success': False, 'msg':
                _('Valid API key needed to create a group')}
