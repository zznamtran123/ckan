...
this.ckan.module('popover-context', function ($, _) {
  return {
     ...
     */
    initialize: function () {
      if (this.options.id != true && this.options.id != null) {
        $.proxyAll(this, /_on/);
        ...
        this.el.popover({
          animation: false,
          html: true,
          content: this.options.throbber.replace('{SITE_ROOT}', this.ckan.SITE_ROOT) + this.i18n('loading'),
          placement: 'bottom'
        });
    ...
    _onRenderPopover: function (html) {
      ...
      if (typeof popover.$tip != 'undefined') {
        ...
        if (follow_check) {
          this.ckan.module.initializeElement(follow_check[0]);
        }
